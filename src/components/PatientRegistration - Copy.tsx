...
export function PatientRegistration() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.age || formData.age.trim() === '' || isNaN(parseInt(formData.age)) || parseInt(formData.age) <= 0) {
      setSubmitError('Age must be a valid positive number');
      return;
    }
    ...
    if (formData.adhaarID && formData.adhaarID.length !== 12) {
      setAdhaarError('Aadhaar ID must be exactly 12 digits');
      setSubmitError('Aadhaar ID must be exactly 12 digits');
      return;
    }
    
    try {
      // Prepare patient data - ensure adhaarID is properly included
      ...
      // Explicitly add adhaarID if it has a value
      if (formData.adhaarID && formData.adhaarID.trim() !== '') {
        patientData.adhaarID = formData.adhaarID.trim();
        console.log('Sending adhaarID to API:', patientData.adhaarID);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to register patient. Please try again.';
            ...
            <div className="overflow-x-auto overflow-y-scroll border border-gray-200 rounded flex-1" style={{ maxHeight: 'calc(100vh - 200px)' }}>
              <table className="w-full">
                ...
                <tbody>
                  ...
                  <tr>
                    <td className="py-1 px-4" colSpan={12}></td>
                  </tr>
                </tbody>
              </table>
            </div>
