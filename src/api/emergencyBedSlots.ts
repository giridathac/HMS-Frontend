  ...
  async getAll(status?: string, emergencyBedId?: number): Promise<EmergencyBedSlot[]> {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        apiData = response.data.map(mapEmergencyBedSlotResponseToEmergencyBedSlot);
      }
    } catch (error) {
    ...
    if (ENABLE_STUB_DATA) {
      ...
      // Merge API data with stub data, avoiding duplicates
      const mergedData = [...apiData];
      stubData.forEach(stubSlot => {
        if (!mergedData.find(slot => slot.emergencyBedSlotId === stubSlot.emergencyBedSlotId)) {
          mergedData.push(stubSlot);
        }
      });
